...
| Gameplay shooting         | `lib/shoot/DieAIGame.as` registers `MouseEvent.CLICK` on a transparent `touchLayer` and shoots toward `mouseX` / `mouseY`.                     | Tap-to-shoot remains unverified at runtime and is not part of the public control copy.                       |
